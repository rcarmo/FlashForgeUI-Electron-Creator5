...
@Test func statusClientReportsTransportFailure() async throws {
    defer {
        DetailTransportFailureMockURLProtocol.handler = nil
    }

    DetailTransportFailureMockURLProtocol.handler = { _ in
        throw URLError(.cannotConnectToHost)
    }

    let configuration = URLSessionConfiguration.ephemeral
    configuration.protocolClasses = [DetailTransportFailureMockURLProtocol.self]
    let client = URLSessionModernPrinterHTTPClient(session: URLSession(configuration: configuration))
...
private final class DetailTransportFailureMockURLProtocol: URLProtocol, @unchecked Sendable {
    static var handler: ((URLRequest) throws -> (HTTPURLResponse, Data))?
